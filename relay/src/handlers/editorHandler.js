...
export const editorHandler = (io, socket) => {
    ...
    socket.on("editor:code_change", async ({ roomId, code, delta }) => {
        try {
            ...
            socket.to(roomId).emit("editor:code_changed", {
                code, delta, userId: socket.userId,
            });
    ...
    // ─── Run Code ─────────────────────────────────────────────────────────────
    socket.on("editor:run_code", async ({ roomId, code, language, questionId }) => {
        try {
            io.to(roomId).emit("editor:execution_started", { userId: socket.userId });
            ...
            console.log(`⚡ Code executed in room ${roomId} — ${language} — ${response.data.executionTimeMs}ms`);
            ...
            io.to(roomId).emit("editor:execution_result", {
                stdout:   "",
                stderr:   err.response?.data?.error || "Execution failed: " + err.message,
                passed:   false,
                timedOut: false,
                results:  [],
            });
    ...
    socket.on("editor:load_question", async ({ roomId, questionId, language }) => {
        try {
            ...
            console.log(`📝 Question loaded: ${questionData.questionTitle} (${currentLanguage})`);
