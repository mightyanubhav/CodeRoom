...
const InterviewRoom = () => {
    ...
    const {
        currentCode, language, question, executionResult,
        isExecuting, setRoomState, setCode, setLanguage,
        setQuestion, setExecuting, setExecutionResult,
        addParticipant, removeParticipant, resetRoom,
    } = useRoomStore();
        ...
        return (
            <div className="min-h-screen bg-[#0d1117] flex items-center justify-center">
                <div className="text-center">
                    <svg className="animate-spin h-10 w-10 text-[#238636] mx-auto mb-4" viewBox="0 0 24 24" fill="none">
        ...
        <div className="h-screen bg-[#0d1117] flex flex-col overflow-hidden">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                {showQuestion && question && (
                    <div className="w-80 bg-[#161b22] border-r border-[#30363d] flex flex-col overflow-hidden flex-shrink-0">
                        <div className="p-4 border-b border-[#30363d]">
                            <div className="flex items-center justify-between mb-1">
                                <h3 className="text-white font-semibold text-sm">{question.questionTitle}</h3>
                                <span className={`text-xs px-2 py-0.5 rounded-full ${
                                    question.difficulty === 'EASY' ? 'text-[#3fb950] bg-[#1a2f1a]' :
                                    question.difficulty === 'MEDIUM' ? 'text-[#d29922] bg-[#2d1f00]' :
                ...
                <div className="flex-1 flex flex-col overflow-hidden">
                    ...
                    {/* Execution result */}
                    {executionResult && (
                        <div className="bg-[#161b22] border-t border-[#30363d] p-3 h-32 overflow-y-auto flex-shrink-0">
                            <div className="flex items-center gap-2 mb-2">
                                <span className={`text-xs font-medium ${executionResult.passed ? 'text-[#3fb950]' : 'text-[#f85149]'}`}>
                                    {executionResult.passed ? '✅ All tests passed' : '❌ Tests failed'}
                                </span>
                            </div>
                            {executionResult.stdout && (
                                <pre className="text-xs text-[#8b949e] font-mono">{executionResult.stdout}</pre>
                            )}
                            {executionResult.stderr && (
                                <pre className="text-xs text-[#f85149] font-mono">{executionResult.stderr}</pre>
                            )}
                        </div>
                    )}
                    ...
                    {/* Executing indicator */}
                    {isExecuting && (
                        <div className="bg-[#161b22] border-t border-[#30363d] p-3 flex-shrink-0">
                            <div className="flex items-center gap-2">
                                <svg className="animate-spin h-3 w-3 text-[#238636]" viewBox="0 0 24 24" fill="none">
                                    <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/>
                                    <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/>
                                </svg>
                ...
                {/* Video panel */}
                {showVideo && (
                    <div className="w-72 bg-[#161b22] border-l border-[#30363d] flex-shrink-0">
