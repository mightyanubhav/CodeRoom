  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        // Update store — triggers useEffect re-run with new token
...
export const interviewAPI = {
  ...
  getByRoomEntityId: (roomEntityId) =>
...
export const questionAPI = {
  ...
  getByDifficulty: (difficulty) =>
    api.get(`/api/questions/difficulty/${difficulty}`),
...
export const roomAPI = {
  ...
  getByInterview: (interviewId) =>
    api.get(`/api/rooms/interview/${interviewId}`),
  loadQuestion: (roomId, questionId) =>
    api.post(`/api/rooms/${roomId}/question/${questionId}`),
