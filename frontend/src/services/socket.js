...
export const initSocket = (token) => {
  // If connected with same token — return existing
  if (socket?.connected) {
    // Check if token changed — if so reconnect
    const currentToken = socket.auth?.token;
    const newToken = `Bearer ${token}`;
    if (currentToken === newToken) return socket;

    // Token changed — disconnect and reconnect
    socket.disconnect();
    socket = null;
  }
  socket = io(RELAY_URL, {
...
export const runCode = (roomId, code, language, questionId) => {
  socket?.emit(SOCKET_EVENTS.EDITOR_RUN_CODE, {
    roomId,
    code,
    language,
    questionId,
