...
const VideoPanel = ({ roomId, participants = [], isLeadInterviewer = false, interviewId = null }) => {
    ...
    useEffect(() => {
        const startWhenReady = () => {
            ...
            if (!socket) { setTimeout(startWhenReady, 100); return; }
    ...
    useEffect(() => {
        const setup = () => {
            const socket = getSocket();
            if (!socket) { setTimeout(setup, 100); return; }
            ...
            socket.on("webrtc:ice_candidate", async ({ candidate, fromUserId }) => {
                ...
                if (pc && candidate) {
                    ...
                    catch (err) { console.warn("ICE error:", err.message); }
                }
            });

            socket.on("webrtc:call_end", ({ fromUserId }) => removePeer(fromUserId));
            socket.on(SOCKET_EVENTS.ROOM_USER_LEFT, ({ userId }) => removePeer(userId));
        };
        ...
        const draw = () => {
            ...
            if (screenVideoElem && screenVideoElem.readyState >= 2) {
                ...
                if (remoteElems.length === 0) {
                    ...
                    const candidateEntry = Object.entries(remoteVideoElemsRef.current).find(([userId]) => {
                        const p = participants.find((p) => p.userId === userId);
                        return p?.role === "CANDIDATE";
                    });
                    const otherElems = Object.entries(remoteVideoElemsRef.current).filter(([userId]) => {
                        const p = participants.find((p) => p.userId === userId);
                        return p?.role !== "CANDIDATE";
                    });
                    ...
                    if (candidateEntry) {
                        ...
                        if (videoElem.readyState >= 2) ctx.drawImage(videoElem, 0, 0, W, topH);
                    } else if (remoteElems[0]?.readyState >= 2) {
                        ctx.drawImage(remoteElems[0], 0, 0, W, topH);
                    }

                    const bottomVideos = [...otherElems.map(([, v]) => v), localVideoRef.current].filter(Boolean);
                    const colW = Math.floor(W / Math.max(bottomVideos.length, 1));
                    bottomVideos.forEach((vid, i) => {
                        if (vid?.readyState >= 2) ctx.drawImage(vid, i * colW, topH, colW, botH);
                    });
                }
    ...
    const handleStartRecording = async () => {
        try {
            ...
            try {
                ...
                await new Promise((res) => { screenVideoElem.onloadedmetadata = res; });
            ...
            setIsRecording(true);
            toast(screenVideoElem ? "Recording started (screen + webcam)" : "Recording started (webcam only)", { icon: "⏺" });
...
const RemoteVideo = ({ stream, small = false }) => {
    ...
    return (
        <div className={`bg-[#0d1117] rounded-lg overflow-hidden ${small ? "aspect-video" : "aspect-video"}`}>
